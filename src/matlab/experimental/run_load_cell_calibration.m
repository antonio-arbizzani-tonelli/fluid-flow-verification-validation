...
leaveOneOutSensitivity = zeros(testCount, 1);
leaveOneOutPredictionError = zeros(testCount, 1);
for testId = 1:testCount
    retained = true(testCount, 1);
    retained(testId) = false;
    leaveOneOutFit = polyfit(referenceForce(retained), steadyVoltage(retained), 1);
    leaveOneOutSensitivity(testId) = leaveOneOutFit(1);
    leaveOneOutPredictionError(testId) = ...
        (steadyVoltage(testId) - leaveOneOutFit(2)) / leaveOneOutFit(1) - referenceForce(testId);
end
leaveOneOutResults = table(referenceForce, leaveOneOutSensitivity, ...
    100 * (leaveOneOutSensitivity / fitCoefficients(1) - 1), ...
    leaveOneOutPredictionError, 'VariableNames', ...
    {'OmittedReferenceForce_N', 'Sensitivity_V_per_N', ...
    'SensitivityChange_percent', 'OmittedPointPredictionError_N'});
writetable(leaveOneOutResults, fullfile(tableDir, 'load_cell_leave_one_out.csv'));
...
fprintf('Leave-one-out sensitivity change: %.4f to %.4f %%.\n', ...
    min(leaveOneOutResults.SensitivityChange_percent), ...
    max(leaveOneOutResults.SensitivityChange_percent));
